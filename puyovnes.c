...
byte destroy_board()
{
  ...
  if (tmp_counter < 6)
  {
    ...
    {
      if ((boards[current_player][tmp_counter][gp_j] & FLAG) > 0)
      {
        //(i+1)<<1 à l'air ok, y on y est pas encore
        //addr = NTADR_A((i+1)<<1, j *2 );//?????
        addr = NTADR_A(((tmp_counter)*2) + nt_x_offset[current_player], gp_j << 1 );//?????
